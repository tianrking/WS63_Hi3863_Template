...
static void print_ip_address(void)
{
    ...
    if (netif == NULL) {
        PRINT("Failed to find netif\r\n");
        return;
    }

    ip_addr = netif->ip_addr.u_addr.ip4;
    netmask = netif->netmask.u_addr.ip4;
    gw = netif->gw.u_addr.ip4;
...
td_s32 example_get_match_network(wifi_sta_config_stru *expected_bss)
{
    ...
    td_char expected_ssid[] = "KUNIU";
    td_char key[] = "kuniu666"; /* 待连接的网络接入密码 */
